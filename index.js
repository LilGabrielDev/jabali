...
async function startBot() {
  ...
  // Check if sessionID is provided and process Lilgabriel! format session
  if (config.sessionID && config.sessionID.startsWith('Lilgabriel!')) {
    try {
      const [header, b64data] = config.sessionID.split('!');

      if (header !== 'Lilgabriel' || !b64data) {
        throw new Error("❌ Invalid session format. Expected 'Lilgabriel!.....'");
      }
      ...
      console.log('📡 Session : 🔑 Retrieved from jabali Session');

    } catch (e) {
      console.error('📡 Session : ❌ Error processing jabali session:', e.message);
